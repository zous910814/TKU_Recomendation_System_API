...
def get_the_most_similar_course(user_id, user_course_matrix, num):
    user_vec = user_course_matrix.loc[user_id].values
    sorted_index = np.argsort(user_vec)[:num]
    return list(user_course_matrix.columns[sorted_index])

a = get_the_most_similar_course(200,ndf_cs,6)
print(ndf.iloc[200])
print(ndf.iloc[a[1:]])