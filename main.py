...
class DNN(nn.Module):
    def __init__(self, inputs_dim, hidden_units, activation='relu', l2_reg=0, dropout_rate=0, use_bn=False,
                 init_std=0.0001, dice_dim=3, seed=1024, device='cuda'):
...
class NFM(nn.Module):
    def __init__(self, feat_sizes, embedding_size, linear_feature_columns, dnn_feature_columns,
                 dnn_hidden_units=(128, 128),
                 l2_reg_embedding=1e-5, l2_reg_linear=1e-5, l2_reg_dnn=0, init_std=0.0001, seed=1024, bi_dropout=1,
                 dnn_dropout=0, dnn_activation='relu', task='binary', device='cuda', gpus=None):
        ...
        self.embedding_dic = nn.ModuleDict(
            {feat.name: nn.Embedding(self.feat_sizes[feat.name], self.embedding_size, sparse=False)
    ...
    def forward(self, X):
        sparse_embedding = [
            self.embedding_dic[feat.name](X[:, self.feature_index[feat.name]].long()).reshape(X.shape[0], 1, -1)
            for feat in self.sparse_features_columns]
...
if __name__ == '__main__':
    ...
    data = pd.read_csv('./data/ndfone.csv', names=col_names, sep='\t')
    # print(data)
    ...
    # print(feat_sizes_dense)
    # print(feat_sizes_sparse)
    feat_sizes.update(feat_sizes_dense)
    feat_sizes.update(feat_sizes_sparse)
    # print(feat_sizes)

    for feat in sparse_features:  # 字串無法套入數學模型進行運算，在此先對其進行Label encoding編碼
        lbe = LabelEncoder()
        data[feat] = lbe.fit_transform(data[feat])
    # print(data[sparse_features])

    nms = MinMaxScaler(feature_range=(0, 1))  # MinMaxScaler 最小最大值標準化
    data[dense_features] = nms.fit_transform(
        data[dense_features])  # 先對partData作fit()的功能，找到該partData的整體統計特性之指標
    # print(data[dense_features])
    ...
    # data.to_csv("data/a.csv", index=False, encoding='utf-8_sig')
    # print(data.head())
